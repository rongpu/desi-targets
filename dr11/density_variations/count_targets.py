# Example:
# python count_targets.py LRG south
...
cat_dir = '/dvs_ro/cfs/cdirs/desi/users/rongpu/targets/dr11.0/5.1.0/resolve'

output_dir = '/global/cfs/cdirs/desi/users/rongpu/data/imaging_sys/dr11/density_maps/5.1.0/resolve'
...
if __name__ == '__main__':
    ...
    cat_path = os.path.join(cat_dir, 'dr11_{}_5.1.0_basic.fits'.format(target_class.lower()))
    cat = Table(fitsio.read(cat_path))
    photsys_mask = cat['PHOTSYS']==photsys
    cat = cat[photsys_mask]
    # pix_path = os.path.join(cat_dir, 'dr11_{}_5.1.0_pixel.fits'.format(target_class.lower()))
    # cat_pix = Table(fitsio.read(pix_path))[photsys_mask]
    # cat = hstack([cat, cat_pix], join_type='exact')
    if custom_mask_name!='':
        mask_path = os.path.join(cat_dir, 'dr11_{}_5.1.0_{}.fits.gz'.format(target_class.lower(), custom_mask_name))
