...
custom_mask_dict = {'LRG': '', 'LGE': '', 'ELG': '', 'ELG_LOP': '', 'QSO': '', 'BGS_ANY': '', 'BGS_BRIGHT': ''}

# maskbits_dict = {'LRG': [], 'LGE': [], 'ELG': [], 'ELG_LOP': [], 'QSO': [1, 8, 9, 11, 12, 13], 'BGS_ANY': [1, 13], 'BGS_BRIGHT': [1, 13]}
...
if target_class=='LGE':
    cat_dir = '/dvs_ro/cfs/cdirs/desicollab/users/rongpu/targets/dr9.0/3.0.0/resolve'
else:
    cat_dir = '/dvs_ro/cfs/cdirs/desicollab/users/rongpu/targets/dr9.0/1.1.1/resolve'

output_dir = '/global/cfs/cdirs/desi/users/rongpu/data/imaging_sys/dr11/density_maps/dr9_for_comparison'
...
if __name__ == '__main__':
    ...
    if target_class=='LGE':
        cat_path = os.path.join(cat_dir, 'dr9_{}_3.0.0_basic.fits'.format(target_class.lower()))
    else:
    ...
    # pix_path = os.path.join(cat_dir, 'dr9_{}_pixel.fits'.format(target_class.lower()))
    # cat_pix = Table(fitsio.read(pix_path))[photsys_mask]
    # cat = hstack([cat, cat_pix], join_type='exact')
    if custom_mask_name!='':
        mask_path = os.path.join(cat_dir, 'dr9_{}_{}.fits.gz'.format(target_class.lower(), custom_mask_name))
    ...
    for nside in nsides:
        ...
        output_path = os.path.join(output_dir, 'counts_{}_{}_nside_{}_minobs_{}_maskbits_{}.fits'.format(sub_class.lower(), field, nside, min_nobs, mask_str))
