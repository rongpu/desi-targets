...
# Only the unWISE maskbits (no HALO or SPIKE)
...
# randoms.write('/global/cfs/cdirs/desi/users/rongpu/tmp/randoms_{}_minobs_{}_maskbits_{}-wise_v1.fits'.format(field, min_nobs, ''.join([str(tmp) for tmp in maskbits])))
...
save_path = '/global/u2/r/rongpu/notebooks/desi_mask/data/new_mask/density_rand_wise_{}_minobs_{}_maskbits_{}.npy'.format(field, min_nobs, ''.join([str(tmp) for tmp in maskbits]))
