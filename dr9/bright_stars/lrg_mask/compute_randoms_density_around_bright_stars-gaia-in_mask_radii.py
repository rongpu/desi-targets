...
gaia['radius'] = gaia['radius_'+field]
...
for index in range(len(gaia_min_list)):
    ...
    search_radius = gaia1['radius'].max() * plot_radius
    ...
    mask_radius = gaia1['radius'][idx1_rand]
...
save_path = '/global/u2/r/rongpu/notebooks/desi_mask/data/lrgmask_v1/density_rand_gaia_{}_minobs_{}_lrgmask_{}-in_mask_radii.npy'.format(field, min_nobs, ''.join([str(tmp) for tmp in lrgmask_bits]))
