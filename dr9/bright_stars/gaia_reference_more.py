# Save more columns (including the DECam mags) for the GAIA+Tycho2 reference catalog
...
tycho2.rename_column('zguess', 'decam_mag_z')

mask = np.isnan(tycho2['decam_mag_z'])
mask_vt = mask & (tycho2['MAG_VT']!=0)
mask_hp = mask & (tycho2['MAG_VT']==0) & (tycho2['MAG_HP']!=0)
tycho2['gaia_g'][mask_vt] = tycho2['MAG_VT'][mask_vt]
tycho2['gaia_g'][mask_hp] = tycho2['MAG_HP'][mask_hp]

tycho2['mask_mag'] = -99
tycho2['mask_mag'] = np.min([tycho2['gaia_g'], tycho2['decam_mag_z']+1], axis=0)
...
tycho2 = tycho2[['RA', 'DEC', 'gaia_g', 'decam_mag_z', 'mask_mag']]
tycho2['decam_mag_g'] = np.nan
tycho2['decam_mag_r'] = np.nan
...
gaia.rename_column('PHOT_G_MEAN_MAG', 'gaia_g')
print(len(gaia))

gaia['mask_mag'] = np.min([gaia['gaia_g'], gaia['decam_mag_z']+1], axis=0)
mask = np.isnan(gaia['decam_mag_z'])
gaia['mask_mag'][mask] = gaia['gaia_g'][mask]
...
mask = np.isnan(gaia['decam_mag_r'])
gaia['decam_mag_r'][mask] = gaia['gaia_g'][mask]

gaia.write('/global/cfs/cdirs/desi/users/rongpu/desi_mask/gaia_reference_dr9_more.fits', overwrite=False)
