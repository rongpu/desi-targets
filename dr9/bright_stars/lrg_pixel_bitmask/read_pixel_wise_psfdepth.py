# Get NOBS_GRZ values for a catalog
# Examples:
# srun -N 1 -C haswell -c 64 -t 04:00:00 -q interactive python read_pixel_wise_psfdepth.py --input catalog.fits --output catalog_wise_psfdepth.fits
# srun -N 1 -C haswell -c 64 -t 04:00:00 -q interactive python read_pixel_wise_psfdepth.py --input /global/cfs/cdirs/desi/target/catalogs/dr9/0.49.0/randoms/resolve/randoms-1-0.fits --output $CSCRATCH/temp/randoms-1-0-wise_psfdepth.fits
# srun -N 1 -C haswell -c 64 -t 04:00:00 -q interactive python read_pixel_wise_psfdepth.py --input /global/cfs/cdirs/desi/users/rongpu/targets/dr9.0/1.0.0/resolve/dr9_lrg_south_1.0.0_basic.fits --output $CSCRATCH/temp/dr9_lrg_south_1.0.0_wise_psfdepth.fits
...
def bitmask_radec(brickid, ra, dec):
    ...
    ivar_w1_fn = os.path.join(bitmask_dir, '{}/coadd/{}/{}/legacysurvey-{}-invvar-W1.fits.fz'.format(field, brickname[:3], brickname, brickname))
    ivar_w2_fn = os.path.join(bitmask_dir, '{}/coadd/{}/{}/legacysurvey-{}-invvar-W2.fits.fz'.format(field, brickname[:3], brickname, brickname))

    ivar_w1 = fitsio.read(ivar_w1_fn)
    ivar_w2 = fitsio.read(ivar_w2_fn)

    header = fits.open(ivar_w1_fn)[1].header
    w = wcs.WCS(header)

    coadd_x, coadd_y = w.wcs_world2pix(ra, dec, 0)
    coadd_x, coadd_y = np.round(coadd_x).astype(int), np.round(coadd_y).astype(int)

    ivar_w1 = ivar_w1[coadd_y, coadd_x]
    ivar_w2 = ivar_w2[coadd_y, coadd_x]

    # Adam's parameters:
    norm = {'W1': 0.240, 'W2': 0.255}
    vega_to_ab = {'W1': 2.699, 'W2': 3.339}

    ff = 10.**(vega_to_ab['W1'] / -2.5)
    psfdepth_w1 = ivar_w1 * norm['W1']**2 / ff**2

    ff = 10.**(vega_to_ab['W2'] / -2.5)
    psfdepth_w2 = ivar_w2 * norm['W2']**2 / ff**2

    return psfdepth_w1, psfdepth_w2
...
def wrapper(bid_index):
    ...
    psfdepth_w1, psfdepth_w2 = bitmask_radec(brickid, ra, dec)

    data = Table()
    data['idx'] = idx
    data['PSFDEPTH_W1'] = psfdepth_w1
    data['PSFDEPTH_W2'] = psfdepth_w2
