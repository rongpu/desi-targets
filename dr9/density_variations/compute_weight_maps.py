...
def write_output(maps):
    ...
    if combine_north_and_south:
        output_path = os.path.join(output_dir, '{}/linear_weights_{}_{}_{}_nside_{}_minobs_{}_maskbits_{}.fits'.format(weights_ver, weights_ver, target_class.lower(), 'combined', nside, min_nobs, ''.join([str(tmp) for tmp in maskbits])+lrgmask_str))
    else:
        output_path = os.path.join(output_dir, '{}/linear_weights_{}_{}_{}_nside_{}_minobs_{}_maskbits_{}.fits'.format(weights_ver, weights_ver, target_class.lower(), field, nside, min_nobs, ''.join([str(tmp) for tmp in maskbits])+lrgmask_str))
    print(output_path)
...
weights_fn_dict = {'LRG': 'main_v0.2/main_lrg_linear_coeffs_v0.2.yaml',
                   'BGS_ANY': 'main_v0.1/main_bgs_any_linear_coeffs_v0.1.yaml',
                   'BGS_BRIGHT': 'main_v0.1/main_bgs_bright_linear_coeffs_v0.1.yaml',
                   'ELG': 'main_v0.1/main_elg_linear_coeffs_v0.1.yaml',
                   'QSO': 'main_v0.1/main_qso_linear_coeffs_separate_des_v0.1.yaml',
                   }
