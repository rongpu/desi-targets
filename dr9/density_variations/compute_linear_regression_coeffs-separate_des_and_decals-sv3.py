...
for target_class in ['BGS_ANY', 'BGS_BRIGHT', 'LRG', 'ELG', 'QSO']:
    ...
    for region in ['BASS+MzLS', 'DECaLS', 'DES']:
        ...
        density = Table.read(os.path.join(target_densities_dir, 'density_map_sv3_{}_{}_nside_{}_minobs_{}_maskbits_{}.fits'.format(target_class, field, nside, min_nobs, ''.join([str(tmp) for tmp in maskbits]))))
    ...
    with open(os.path.join(output_dir, "sv3_{}_linear_coeffs_separate_des_v0.1.yaml".format(target_class)), "w") as f:
