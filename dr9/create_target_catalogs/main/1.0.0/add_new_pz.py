# Add the new photo-z's that were trained using DESI redshifts
# Example:
# srun -N 1 -C haswell -c 64 -t 04:00:00 -L cfs -q interactive python add_new_pz.py LRG south
...
pz_dir = '/global/cfs/cdirs/desi/users/rongpu/ls_dr9.0_desi_photoz/pz/{}'.format(field)

cat_basic_path = os.path.join(data_dir, 'dr9_{}_{}_1.0.0_basic.fits'.format(target_class.lower(), field))
output_path = os.path.join(data_dir, 'dr9_{}_{}_1.0.0_pz_new.fits'.format(target_class.lower(), field))
...
def get_sweep_columns(sweep_fn):
    ...
    pz_fn = os.path.join(pz_dir, pz_fn)
