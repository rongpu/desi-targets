...
input_paths = [
    '/dvs_ro/cfs/cdirs/desicollab/users/rongpu/targets/dr9.0/zp_offset_corrected/dr9_bgs_basic.fits',
]
...
for input_path in input_paths:
    ...
    output_path = input_path.replace('_basic.fits', '_gaia_dr3.fits').replace('/dvs_ro/', '/global/')

    cat = Table(fitsio.read(input_path))
    tmp = Table(fitsio.read(input_path.replace('_basic.fits', '_more_1.fits'), columns=['REF_EPOCH', 'PMRA', 'PMDEC']))
