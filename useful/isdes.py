...
#     # checking hp pixels
#     mng = pymangle.Mangle(os.path.expanduser('/global/cfs/cdirs/desi/users/rongpu/useful/in_des/des.ply'))
...
#     cat = Table()
#     cat['in_des'] = indes
#     cat.write(os.path.expanduser('/global/cfs/cdirs/desi/users/rongpu/useful/in_des/hp_in_des_{}_ring.fits.gz'.format(nside)))


def get_isdes(ra, dec, nside=256):

    hpix = hp.ang2pix(nside, ra, dec, nest=False, lonlat=True)
    cat = Table.read(os.path.expanduser('/global/cfs/cdirs/desi/users/rongpu/useful/in_des/hp_in_des_{}_ring.fits.gz'.format(nside)))
