# Create combined SV3 cumulative BGS catalog
...
for tileid in tileid_list:
    ...
    for fn in fn_list:
        ...
        mask = tmp['SV3_DESI_TARGET'] & 2**60 > 0
        tmp = tmp[mask]
...
bgs = []
for field in ['north', 'south']:
    bgs.append(Table(fitsio.read(os.path.join(main_dir, 'dr9_bgs_any_{}_1.0.0_basic.fits'.format(field)), columns=['TARGETID', 'BGS_TARGET'])))
bgs = vstack(bgs)
...
cat.write('/global/cfs/cdirs/desi/users/rongpu/spectro/fugu/sv3_{}_bgs.fits'.format(coadd_type), overwrite=True)
