...
mask = (tiles['TARGETS']=='ELG') | (tiles['TARGETS']=='QSO+ELG')
tiles = tiles[mask]
print(len(tiles))
...
for tileid in tileid_list:
    ...
    for fn in fn_list:
        ...
        mask = tmp['SV1_DESI_TARGET'] & 2**1 > 0
        tmp = tmp[mask]
...
for field in ['north', 'south']:
    tmp = Table(fitsio.read(os.path.join(sv1_dir, 'dr9_sv1_elg_{}_0.49.0_basic.fits'.format(field)), columns=['TARGETID']))
    tmp1 = Table(fitsio.read(os.path.join(sv1_dir, 'dr9_sv1_elg_{}_0.49.0_elgmask_v1.fits'.format(field))))
    elg.append(hstack([tmp, tmp1]))
elg = vstack(elg)

mask = np.in1d(elg['TARGETID'], cat['TARGETID'])
elg = elg[mask]

cat = join(cat, elg, keys='TARGETID')
...
cat.write('/global/cfs/cdirs/desi/users/rongpu/spectro/fugu/sv1_{}_elg.fits'.format(coadd_type), overwrite=True)
