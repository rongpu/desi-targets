...
# mask = tiles['LASTNIGHT']>=20210917  # dates of reprocessing
mask &= tiles['LASTNIGHT']<=20211205  # dates of reprocessing
tiles = tiles[mask]
print(len(tiles))
...
for index in range(len(tiles)):
    ...
    fn_list = sorted(glob.glob(os.path.join(data_dir, str(tileid), str(tiles['LASTNIGHT'][index]), 'redrock-*.fits')))
    for fn in fn_list:
        ...
        mask = tmp['DESI_TARGET'] & 2**0 > 0  # Select LRGs
        tmp = tmp[mask]
...
output_path = '/global/cfs/cdirs/desi/users/rongpu/spectro/daily/main_done_{}_lrg_good_postshutdown_to_20211205.fits'.format(coadd_type)
print(output_path)
