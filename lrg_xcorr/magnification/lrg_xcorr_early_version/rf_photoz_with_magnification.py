...
def compute_photoz(split_idx, kf_index, magnification):
    ...
    for tree_index in range(n_estimators):
        ...
        for perturb_index in range(n_perturb):
            ...
            data1 = np.column_stack((gmag1-rmag1, rmag1-zmag1, zmag1-w1mag1, w1mag1-w2mag1, rmag1, radius1, q, p))
...
if __name__ == '__main__':
    ...
    output_path = '/global/cfs/cdirs/desi/users/rongpu/data/lrg_xcorr/magnification/lrg_magnification_pz_{}_{:g}.fits'.format(field, magnification)
    chunks_dir = '/global/cfs/cdirs/desi/users/rongpu/data/lrg_xcorr/magnification/tmp/kf_chunks/'+field

    cat_all = Table(fitsio.read('/global/cfs/cdirs/desi/users/rongpu/data/lrg_xcorr/magnification/lrg_magnification_{}.fits'.format(field), columns=columns))
    cat_size = len(cat_all)

    np.random.seed(1456+cat_size)
    kf_idx = np.random.choice(5, size=cat_size)

    cat_all['id'] = np.array(cat_all['OBJID'], dtype='int64') * int(1e10) \
              + np.array(cat_all['BRICKID'], dtype='int64') * int(1e4) \
              + np.array(cat_all['RELEASE'], dtype='int64')

    # Full spec-z truth catalog (not all objects were used for training):
    specz_full_path = '/global/cfs/cdirs/desi/users/rongpu/ls_dr9.0_desi_photoz/truth/truth_combined_dr9.0_desi_20210718_{}.fits'.format(field)
    # Catalog used for training
    specz_train_path = '/global/cfs/cdirs/desi/users/rongpu/ls_dr9.0_desi_photoz/truth/truth_combined_dr9.0_desi_20210718_{}_training.fits'.format(field)

    specz_train = Table(fitsio.read(specz_train_path, columns=['id', 'redshift', 'survey', 'kfold']))
    # specz_train['survey'] = np.array(specz_train['survey']).astype('str')
    train_id_all = np.array(specz_train['id'])
    ...
    for kf_index in range(5):
        ...
        test_id = np.array(specz_train['id'][mask])

        mask_kf = (kf_idx==kf_index) & (~np.in1d(cat_all['id'], train_id_all))  # randomly assigned KF membership
        mask_train = np.in1d(cat_all['id'], test_id)  # objects in the spec-z KF test set
        ...
        tree_dir = '/global/cfs/cdirs/desi/users/rongpu/ls_dr9.0_desi_photoz/individual_trees/20210718_5-fold-{}/fold-{}'.format(field, kf_index+1)

        # Load all the single pre-trained trees
        regrf_all = []
        for tree_index in range(n_estimators):
            regrf_all.append(joblib.load(os.path.join(tree_dir, 'regrf_20210718_{:d}.pkl'.format(tree_index))))
    ...
    pz['id'] = np.array(pz['OBJID'], dtype='int64') * int(1e10) \
              + np.array(pz['BRICKID'], dtype='int64') * int(1e4) \
              + np.array(pz['RELEASE'], dtype='int64')

    # Here matching tt to cat_all
    if len(cat_all)!=len(pz) or not np.all(np.unique(cat_all['id'])==np.unique(pz['id'])):
        raise ValueError
    t1_reverse_sort = np.array(cat_all['id']).argsort().argsort()
    pz = pz[np.argsort(pz['id'])[t1_reverse_sort]]
