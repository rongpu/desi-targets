...
sys.path.append(os.path.expanduser('~/git/pixel_partition'))
import pp_modules
# Or just copy these lines: https://github.com/rongpu/pixel_partition/blob/master/pp_modules.py#L36C5-L113

nside = 256
randoms['pix'] = hp.ang2pix(nside, randoms['RA'], randoms['DEC'], nest=False, lonlat=True)

pix_unique = np.unique(randoms['pix'])
ra, dec = hp.pix2ang(nside, pix_unique, nest=False, lonlat=True)
pix_labels = pp_modules.identify_islands(nside, ra, dec)

tmp = Table()
tmp['pix_labels'], tmp['count'] = np.unique(pix_labels, return_counts=True)
tmp.sort('count', reverse=True)
pix_labels_to_keep = np.array(tmp['pix_labels'][:2])  # keeping only the two "continents"
pix_to_keep = np.array(pix_unique[np.in1d(pix_labels, pix_labels_to_keep)])
np.save('/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/catalogs/dr10_photoz/misc/not_islands_pix_{}.npy'.format(nside), pix_to_keep)
