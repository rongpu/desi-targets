...
for bin_index in range(1, 5):
    ...
    msk = hp.read_map('/global/cfs/cdirs/desicollab/users/rongpu/lrg_xcorr/data_products/maps/main_lrg/lrg_s0{}_msk.hpx2048.fits'.format(bin_index))
    lrg = hp.read_map('/global/cfs/cdirs/desicollab/users/rongpu/lrg_xcorr/data_products/maps/main_lrg/lrg_s0{}_del.hpx2048.fits'.format(bin_index))
    pix_list = np.arange(hp.nside2npix(nside_in))

    mask = msk>0
    lrg = lrg[mask]
    pix_list = pix_list[mask]
    hp_table = Table()
    hp_table['HPXPIXEL'] = pix_list
    hp_table['delta'] = lrg

    newtable = downsize_hp_map(nside_in, nside_out, hp_table, n_processes=n_processes)
    newtable.write('/pscratch/sd/r/rongpu/tmp/lrg_bin_{}_{}.fits'.format(bin_index, nside_out))
