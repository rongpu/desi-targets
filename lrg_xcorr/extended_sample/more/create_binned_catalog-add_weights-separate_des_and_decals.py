...
if include_ebv:
    weights_path = '/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/imaging_weights/extended_lrg/separate_des_and_decals/extended_lrg_linear_coeffs_pz.yaml'
    output_fn = '/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/catalogs/more/dr9_extended_lrg_pzbins_20230509-weights-separate_des_and_decals.fits'
else:
    weights_path = '/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/imaging_weights/extended_lrg/separate_des_and_decals/extended_lrg_linear_coeffs_pz_no_ebv.yaml'
    output_fn = '/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/catalogs/more/dr9_extended_lrg_pzbins_20230509-weights_no_ebv-separate_des_and_decals.fits'

time_start = time.time()

# Load LRG catalog
cat = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/data/lrg_xcorr/catalogs/dr9_extended_lrg_pzbins_20230509.fits'))
more_2 = Table(fitsio.read('/global/cfs/cdirs/desicollab/users/rongpu/targets/dr9.0/zp_offset_corrected/dr9_extended_lrg_more_2.fits',
...
cat['region'] = '        '
mask = cat['PHOTSYS']=='N'
cat['region'][mask] = 'BASSMzLS'
nside = 256
isdes = get_isdes(cat['RA'], cat['DEC'], nside)
mask = (cat['PHOTSYS']=='S') & isdes
cat['region'][mask] = 'DES'
mask = (cat['PHOTSYS']=='S') & (~isdes)
cat['region'][mask] = 'DECaLS'

for region in ['BASSMzLS', 'DECaLS', 'DES']:
    ...
    for bin_index in range(1, 5):  # 4 bins
        ...
        mask_bin = cat['region']==region
        mask_bin &= cat['pz_bin']==bin_index
        cat1 = cat[mask_bin].copy()

        xnames_fit = list(linear_coeffs['DECaLS_bin_1'].keys())
