...
for region in ['DECaLS', 'DES']:
    ...
    for magnification in [0.99, 1., 1.01]:
        ...
        counts['{}_all_{:.3f}'.format(region, magnification)] = int(np.sum(mask_quality & mask_lrg))
        ...
        for bin_index in range(len(pz_cuts)-1):
            ...
            counts['{}_bin_{}_{:.3f}'.format(region, bin_index+1, magnification)] = int(np.sum(mask_quality & mask_lrg & mask_pz))

fn = 'extended_lrg_counts_decals_vs_des'
