...
pz_cuts_south = [0.860, 0.920, 1.00]
pz_cuts_north = [0.854, 0.920, 1.01]
...
for field in ['north', 'south']:
    ...
    for magnification in [0.99, 1., 1.01]:
        ...
        for bin_index in range(len(pz_cuts)-1):
            ...
            counts['{}_bin_4_{}_{:.3f}'.format(field, bin_index+1, magnification)] = int(np.sum(mask_quality & mask_lrg & mask_pz))
...
with open('/global/cfs/cdirs/desi/users/rongpu/lrg_xcorr/magnification/counts/split_bin_4/{}.txt'.format(fn), "w") as f:
    yaml.dump(counts, f)
