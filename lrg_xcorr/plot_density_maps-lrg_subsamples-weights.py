...
top_plot_dir = os.path.join(top_plot_dir, 'weight_maps')

dpi_dict = {64: 200, 128: 200, 256: 600, 512: 1600}
# xsize_dict = {64: 8000, 128: 8000, 256: 12000, 512: 16000}
xsize_dict = {64: 4000, 128: 8000, 256: 12000, 512: 16000}
...
for bin_index in range(1, 5):
    ...
    # for nside in [64]:
        ...
        plot_path = os.path.join(plot_dir, 'weights_lrg_pz_bin_{}_{}.png'.format(bin_index, nside))
        # if os.path.isfile(plot_path):
        #     continue

        for field in ['north', 'south']:

            density_original = Table(fitsio.read(os.path.join(target_densities_dir, 'density_map_lrg_pz_bin_{}_{}_nside_{}_minobs_{}.fits'.format(bin_index, field, nside, min_nobs))))
            density_weighted = Table(fitsio.read(os.path.join(target_densities_dir, 'linear_weights', 'density_map_lrg_pz_bin_{}_{}_nside_{}_minobs_{}-lw.fits'.format(bin_index, field, nside, min_nobs))))
            assert len(density_original)==len(density_weighted) and np.all(density_original['HPXPIXEL']==density_weighted['HPXPIXEL'])
            density_weighted['n_targets_original'] = np.array(density_original['n_targets'], dtype=float)
            ...
            maps = join(maps, density_weighted[['HPXPIXEL', 'n_targets', 'n_targets_original']], join_type='outer', keys='HPXPIXEL').filled(0)
        ...
        maps_overlap['n_targets_original'] = maps_overlap_north['n_targets_original'] + maps_overlap_south['n_targets_original']
        ...
        average_density = np.sum(maps['n_targets_original']) / (np.sum(maps['FRACAREA'])*pix_area)
        ...
        maps['weights'] = maps['n_targets'] / (maps['n_targets_original']/average_density)

        vmin, vmax = 0.9, 1.1

        plot_map(nside, maps['weights'], pix=maps['HPXPIXEL'],
                 vmin=vmin, vmax=vmax, xsize=xsize_dict[nside], dpi=dpi_dict[nside], cmap='viridis',
