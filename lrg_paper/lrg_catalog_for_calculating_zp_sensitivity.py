# Based on useful/create_lrg_catalog.py
# Relax all cuts by 0.02 mag
# The resulting catalog should be the same as Adam's official LRG catalog

# srun -N 1 -C haswell -c 64 -t 04:00:00 -q interactive python lrg_catalog_for_calculating_zp_sensitivity.py
...
def get_lrgs(sweep_path):
    ...
    mask_quality &= cat['FIBERTOTFLUX_Z'] < 10**(-0.4*(17.5-22.5)) + dmag
    ...
    if '/dr9/south/sweep/' in sweep_path:
        mask_lrg &= zmag - w1mag > 0.8 * (rmag - zmag) - 0.6 - dmag  # non-stellar cut
        mask_lrg &= zfibermag < 21.6 + dmag                  # faint limit
        mask_lrg &= (gmag - w1mag > 2.9 - dmag) | (rmag - w1mag > 1.8 - dmag)  # low-z cuts
        mask_lrg &= (
            ((rmag - w1mag > (w1mag - 17.14) * 1.8 - dmag)
             & (rmag - w1mag > (w1mag - 16.33) * 1. - dmag))
            | (rmag - w1mag > 3.3 - dmag)
        )  # double sliding cuts and high-z extension
    elif '/dr9/north/sweep/' in sweep_path:
        mask_lrg &= zmag - w1mag > 0.8 * (rmag - zmag) - 0.6 - dmag  # non-stellar cut
        mask_lrg &= zfibermag < 21.61 + dmag                  # faint limit
        mask_lrg &= (gmag - w1mag > 2.97 - dmag) | (rmag - w1mag > 1.8 - dmag)  # low-z cuts
        mask_lrg &= (
            ((rmag - w1mag > (w1mag - 17.13) * 1.83 - dmag)
             & (rmag - w1mag > (w1mag - 16.31) * 1. - dmag))
            | (rmag - w1mag > 3.4 - dmag)
        )  # double sliding cuts and high-z extension
    else:
        raise ValueError
...
cat.write('/global/cfs/cdirs/desi/users/rongpu/misc/desi_lrg_catalog_for_calculating_zp_sensitivity.fits')
