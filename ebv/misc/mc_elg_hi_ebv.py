...
tmp_dir = '/pscratch/sd/r/rongpu/imaging_mc/tmp1/'
...
def quicksim(truth, cat):
    ...
    for band in fiberflux_bands:
        ...
        sim['fiberflux_'+band] = np.array(truth['fiberflux_{}_ec'.format(band)] / 10**(0.4*ext_coeffs[band]*cat['ebv']) + noise[band+'fiber'] * fiberflux_ratio*flux_err[band], dtype='float32')

    for band in ['g', 'r', 'z']:
        sim['flux_'+band] = np.array(truth['flux_{}_ec'.format(band)] / 10**(0.4*ext_coeffs[band]*cat['ebv']) + noise[band] * flux_err[band], dtype='float32')
...
ebv_nside = 1024
ebv['pix'] = np.arange(hp.nside2npix(ebv_nside))
...
truth['pix'] = hp.ang2pix(ebv_nside, truth['ra'], truth['dec'], nest=False, lonlat=True)
truth = join(truth, ebv, keys='pix', join_type='inner')
print('truth', len(truth))
truth['ebv'] = truth['EBV_HI']
...
for randoms_path in randoms_paths:
    ...
    cat['pix'] = hp.ang2pix(ebv_nside, cat['RA'], cat['DEC'], nest=False, lonlat=True)
    mask = np.in1d(cat['pix'], ebv['pix'])
    cat = cat[mask]
    cat = join(cat, ebv, keys='pix', join_type='inner')
    print(len(cat))
    cat['EBV'] = cat['EBV_HI']
...
hp_table.write('/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20230201/mc_elg_hi_ebv_elgmask_v1_healpix_{}.fits'.format(nside), overwrite=True)
