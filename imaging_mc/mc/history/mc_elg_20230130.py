...
def quicksim(truth, cat):
    ...
    for band in ['g', 'r', 'z']:
        if band in fiberflux_bands:
            ...
            # PSF objects' fiberflux is perfectly correlated with flux
            ...
            noise_flux = np.random.randn(len(cat))
...
for randoms_path in randoms_paths:
    ...
    output_path = '/pscratch/sd/r/rongpu/imaging_mc/tmp2/mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-elgmask_v1.fits')
...
hp_table.write('/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20230130/mc_elg_randoms_elgmask_v1_healpix_{}.fits'.format(nside), overwrite=True)
