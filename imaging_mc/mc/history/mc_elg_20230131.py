...
n_processes = 64
repeats = 256  # number of MC sims for each random point
...
flux_scale = 1.25
fiberflux_scale = 1.1
flux_fiberflux_cov = 0.65 * flux_scale * fiberflux_scale

tmp_dir = '/pscratch/sd/r/rongpu/imaging_mc/tmp3/'
...
def quicksim(truth, cat):
    ...
    for band in ['g', 'r', 'z']:
        if band in fiberflux_bands:
            mean = [0, 0]
            # parameters for not-PSF ELGs
            cov = [[flux_scale**2, flux_fiberflux_cov],
                   [flux_fiberflux_cov, fiberflux_scale**2]]
            noise_flux, noise_fiberflux = np.random.multivariate_normal(mean, cov, len(cat)).T
            # Most PSF objects have fiberflux perfectly correlated with flux
            noise_fiberflux[truth['is_psf']] = noise_flux[truth['is_psf']]
            fiberflux_ratio = truth['fiberflux_{}_ec'.format(band)] / truth['flux_{}_ec'.format(band)]
            sim['fiberflux_'+band] = np.array(truth['fiberflux_{}_ec'.format(band)] / 10**(0.4*ext_coeffs[band]*cat['ebv']) + noise_fiberflux * fiberflux_ratio*flux_err[band], dtype='float32')
        else:
            noise_flux = flux_scale * np.random.randn(len(cat))
        sim['flux_'+band] = np.array(truth['flux_{}_ec'.format(band)] / 10**(0.4*ext_coeffs[band]*cat['ebv']) + noise_flux * flux_err[band], dtype='float32')
...
hp_table.write('/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20230131/mc_elg_randoms_elgmask_v1_healpix_{}.fits'.format(nside), overwrite=True)
