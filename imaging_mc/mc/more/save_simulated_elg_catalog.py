...
def quicksim(truth, cat):
    ...
    sim = Table()
    sim = cat[['ra', 'dec', 'nobs_g', 'nobs_r', 'nobs_z']].copy()
    ...
    for band in ['g', 'r', 'z']:
        ...
        sim['flux_ivar_'+band] = pix_ivar/nea[band]
...
def elgsim(foo):
    ...
    sim['elglop'] = elglop.copy()
    sim['elgvlo'] = elgvlo.copy()

    mask = elglop | elgvlo
    sim = sim[mask]

    sim.remove_columns(['gmag', 'rmag', 'zmag', 'gfibermag'])
...
truth = truth[['ra', 'dec', 'flux_g_ec', 'flux_r_ec', 'flux_z_ec', 'fiberflux_g_ec', 'shape_r']]
...
output_path = '/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20230127/test/mc_elg_catalog.fits'
...
sim = vstack(sim_stack)
sim.write(output_path, overwrite=True)
